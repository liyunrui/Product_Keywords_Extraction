...
class F1Optimizer():
    ...
    def maximize_expectation(P, pNone=None):
        ...
        parameters:
        ------------
        predNone: Boolean value
        best_k: the best index
        max_f1: maximum of expected f1 score [0,1]
        '''                
...
def get_best_prediction(items, preds, pNone=None):
    ...
    items: list
    preds: lit
    P: sorted probability that is keywords. given product name and extracted words by jieba
    L: item corresponding to the above probability
    opt[2]: maximum of expected f1 score [0,1] give the prob and item.
    opt[1]: True of False. True means there is no extracted keywords.
    opt[0]: What's the best threshold(indicator) given the prob and items.
    '''
    ...
    return (list(map(str,best_prediction)))

if __name__ == '__main__':
    print (get_best_prediction(['a', 'b'], [0.9, 0.3]))
